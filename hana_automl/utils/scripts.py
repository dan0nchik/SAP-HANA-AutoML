...
def setup_user(connection_context: ConnectionContext, username: str, password: str):
    ...
    print("AFL_AREAS", connection_context.sql("""SELECT * FROM "SYS"."AFL_AREAS" WHERE AREA_NAME = 'AFLPAL';""").count() > 0)
    print("AFL_PACKAGES", connection_context.sql("""SELECT * FROM "SYS"."AFL_PACKAGES" WHERE AREA_NAME = 'AFLPAL';""").count() > 0)
    print("AFL_FUNCTIONS", connection_context.sql("""SELECT * FROM "SYS"."AFL_FUNCTIONS" WHERE AREA_NAME = 'AFLPAL';""").count() > 0)
    print('Checking if user exists...')
    users = connection_context.sql(f'SELECT USER_NAME FROM "SYS"."USERS"').collect()
    if username not in list(users['USER_NAME']):
        print(f'User {username} does not exist, creating new user with default password '
    ...
    cursor.execute(f"""GRANT "AFL__SYS_AFL_AFLPAL_EXECUTE_WITH_GRANT_OPTION" TO {username}""")
    print('Done.')
