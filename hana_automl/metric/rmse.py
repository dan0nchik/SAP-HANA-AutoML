...
def rmse_score(algo=None, df: DataFrame = None, target=None, ftr: list = None, id: str = None):
    if algo is not None:
        ...
        pandas = res.collect()
        cols = res.columns
        pandas['rmse_coef'] = pandas.apply(lambda row: (Decimal(row[cols[2]]) - Decimal(row[cols[1]])) ** 2, axis=1)
        return math.sqrt(pandas['rmse_coef'].mean())
    else:
        pandas = df.collect()
        cols = df.columns
        pandas['rmse_coef'] = pandas.apply(lambda row: (Decimal(row[cols[0]]) - Decimal(row[cols[1]])) ** 2, axis=1)
        return math.sqrt(pandas['rmse_coef'].mean())
