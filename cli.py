...
@click.option('--optimizer', default="BayesianOptimizer", help='Optimizer that will find the best algorithm')
@click.option('--wizard', default=False, help='Interactive mode. Best for beginners')
def start(i, target, rm_columns, categorical, id_column, optimizer, steps, wizard, table):
    if wizard:
        wizard_mode()
        return
    automl = AutoML()
...
def wizard_mode():
    ...
    for col in df.columns:
        ...
        col_index += 1
    target = df.columns[int(input("Enter number of column(s) that you want to predict. (Example: 3,4 or 5): "))]
    ...
    for col in df.columns:
        print(f"[{col_index}]", col)
        col_index += 1
    id_column = df.columns[int(input("Enter number of column to set as ID column. (Example: 3,4 or 5): "))]
    ...
    print("We've automatically detected categorical (string, date, object, etc) columns:")
    col_index = 0
    for col in df.columns:
        print(f"[{col_index}]", "categ." if df[col].dtype == str or df[col].dtype == np.object_ else "normal",
              f"dtype: {df[col].dtype}", "name: ", col)
        col_index += 1
    ...
    table = input("Great. Now we need to load your data to HANA. Enter name if you have existing table ([Enter] for "
                  "none): ")

    print("Starting automated machine learning...")
    automl = AutoML()
    automl.fit(df, target=target, table_name=table, columns_to_remove=col_list, categorical_features=cat_list, id_column=id_column)
